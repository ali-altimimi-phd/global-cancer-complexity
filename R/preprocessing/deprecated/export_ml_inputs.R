...
export_ml_inputs_from_eset <- function(eset_list,
  ...
  # Extract GSM IDs from expression column names
  ...
  if (any(is.na(match_idx))) {
    missing <- expr_sample_ids[is.na(match_idx)]
    stop(
      sprintf(
  ...
  # Restore CEL-based sample IDs for downstream consistency
  ...
  if (!identical(rownames(meta), colnames(expr))) {
    stop("Metadata alignment failed: rownames(metadata) do not match colnames(expression).", call. = FALSE)
  }
  ...
  meta_export$sample_id <- rownames(meta_export)
  ...
  if (isTRUE(run_pca_check)) {
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      p <- ggplot2::ggplot(pca_df, ggplot2::aes(x = PC1, y = PC2, color = condition)) +
        ...
        ggplot2::labs(
          title = sprintf("PCA sanity check: %s", chip_id),
      ...
      pca_plot_path <- file.path(plot_dir, sprintf("%s_pca_sanity_top%d_%s.png", chip_id, top_n, filter_method))
      ggplot2::ggsave(pca_plot_path, p, width = 8, height = 6, dpi = 300)
      log_it(sprintf("Wrote PCA sanity plot: %s", pca_plot_path), section = "PCA")
    } else {
