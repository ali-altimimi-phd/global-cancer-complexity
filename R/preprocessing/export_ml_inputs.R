...
#' writes full and variance-filtered expression matrices, and optionally runs
#' a PCA sanity check.
#'
...
export_ml_inputs_from_eset <- function(eset_list,
  ...
  # --- Validate / defensively repair metadata alignment ---
  expr_sample_ids <- colnames(expr)
  
  # In the canonical saved ExpressionSet, metadata rownames should already match
  # expression column names. If not, fall back to GSM-based matching for backward
  # compatibility with older saved objects.
  ...
  if (!identical(filter_method, "variance")) {
    stop(
      sprintf(
        "Unsupported filter_method: %s. Currently supported: 'variance'.",
        filter_method
      ),
      call. = FALSE
    )
  ...
  top_expr_path <- file.path(output_dir,
                             sprintf("%s_expr_top%d_%s.csv", chip_id, top_n, filter_method))
  top_features_path <- file.path(output_dir,
                                 sprintf("%s_top%d_%s_features.csv", chip_id, top_n, filter_method))
  ...
  if (isTRUE(run_pca_check)) {
    ...
    if (requireNamespace("ggplot2", quietly = TRUE)) {
      ...
      pca_plot_path <- file.path(plot_dir,
                                 sprintf("%s_pca_top%d_%s.png", chip_id, top_n, filter_method))
      ggplot2::ggsave(
        pca_plot_path,
        p,
        width = 8,
