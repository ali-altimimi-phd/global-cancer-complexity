...
run_all_reports <- function(summaries_combined_df,
                            ...
                            plot_output_dir = "quarto/resources/plots",
                            table_output_dir = "quarto/resources/tables",
                            qmd_output_dir = "quarto/reports/generated_reports") {
  library(glue)
  ...
  dir.create(qmd_output_dir, recursive = TRUE, showWarnings = FALSE)
  ...
  for (cmp in all_comparisons) {
    ...
    new_lines <- c(
      param_yaml,
      template_lines[(header_end + 1):length(template_lines)]
    )

    output_qmd <- file.path(qmd_output_dir, glue("comparison_report_{clean_cmp}.qmd"))
